...
class Caustics extends Component {
    ...
    constructor(base, config = {}) {
        super(base);
        const { lightSource = new THREE.Vector3(-10, 13, -10), intensity = 0.5, normalMaterial = new THREE.MeshNormalMaterial(), ior = 1.25, chromaticAberration = 0.16, samples = 16, saturation = 1.265, noiseIntensity = 0.01, scaleCorrection = 1.75, } = config;
    ...
    adjustPlane() {
        ...
        const newVertices = boundsVertices.map((vert) => vert.add(lightDir.clone().multiplyScalar(-vert.y / lightDir.y)));
    ...
    update() {
        ...
        this.causticsPlane.material.uniforms.uTexture.value = this.causticsFBO.rt.texture;
